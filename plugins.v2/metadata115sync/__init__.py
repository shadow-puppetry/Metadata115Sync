...
class Metadata115Sync(_PluginBase):
    """把 NAS 本地已有的元数据增量同步到 MoviePilot 已配置的 115。"""
        ...
        return [
            {
                ...
                "content": [
                    ...
                    {"component": "VTextField", "props": {"model": "extensions", "label": "元数据扩展名", "hint": "默认：.nfo,.jpg,.jpeg,.png,.webp,.xml"}},
                    {"component": "VTextField", "props": {"model": "max_size_mb", "label": "元数据大小上限（MB）", "type": "number", "min": 1, "max": 1024}},
                    {"component": "VSwitch", "props": {"model": "parent_metadata", "label": "父目录元数据同步"}},
                    {"component": "VTextField", "props": {"model": "interval_minutes", "label": "自动同步间隔（分钟）", "type": "number", "min": 5, "max": 10080}},
                ],
            }
        ], {
            "enabled": False,
    ...
    def get_page(self) -> list[dict]:
        return [
            ...
            {"component": "VAlert", "props": {"type": "secondary", "variant": "tonal", "text": "本插件只执行 NAS → 115 单向补齐：115 已存在的文件跳过，不覆盖、不删除、不使用 TMDB。"}},
        ]
    ...
    def _upload_one(self, local_file: Path, local_root: Path, remote_root: str) -> str:
        ...
        uploaded = chain.upload_file(fileitem=parent, path=local_file, new_name=local_file.name)
        if not uploaded:
    ...
    def sync(self) -> dict[str, int]:
        ...
        try:
            ...
            self._last_result = f"同步完成：扫描 {stats['scanned']}，上传 {stats['uploaded']}，已存在跳过 {stats['skipped']}，失败 {stats['failed']}"
            return stats
        finally:
            self._lock.release()
